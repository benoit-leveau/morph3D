...
EXT_C DMat svdNewDMat(int rows, int cols);
/* Frees a dense matrix. */
EXT_C void svdFreeDMat(DMat D);
...
EXT_C SMat svdConvertDtoS(DMat D);

/* Transposes a dense matrix (returning a new one) */
EXT_C DMat svdTransposeD(DMat D);
...
EXT_C SVDRec svdLAS2A(SMat A, long dimensions);
